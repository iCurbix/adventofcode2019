...
def getinstruction(a):
    l = len(a)
    if l < 5:
        a = '0' * (5 - l) + a
    return a[-2:], a[2:3], a[1:2], a[0:1], paramnum[a[-2:]]


def getval(pos, type):
    if type == '0':
        return inpt[int(inpt[pos])]
    if type == '1':
        return inpt[pos]


def writeval(pos, val):
    inpt[int(inpt[pos])] = val


def code01(ptr, type1, type2, type3):
    a = int(getval(ptr + 1, type1))
    b = int(getval(ptr + 2, type2))
    writeval(ptr + 3, str(a + b))
    return 0


def code02(ptr, type1, type2, type3):
    a = int(getval(ptr + 1, type1))
    b = int(getval(ptr + 2, type2))
    writeval(ptr + 3, str(a * b))
    return 0
...
def code03(ptr, type1, type2, type3):
    a = input()
    writeval(ptr + 1, a)
    return 0


def code04(ptr, type1, type2, type3):
    outs.append(getval(ptr + 1, '0'))
    return 0


def code05(ptr, t1, t2, t3):
    global pointer
    if getval(ptr + 1, t1) != '0':
        pointer = int(getval(pointer + 2, t2))
        return 1
    return 0


def code06(ptr, t1, t2, t3):
    global pointer
    if getval(ptr + 1, t1) == '0':
        pointer = int(getval(pointer + 2, t2))
        return 1
...
def code99(ptr, type1, type2, type3):
    print(outs)
    global running
    running = False
    return 0
...
def part1():
    ...
    while running:
        ...
        if instruction[opcode](pointer, type1, type2, type3) == 0:
            pointer += pnum + 1
